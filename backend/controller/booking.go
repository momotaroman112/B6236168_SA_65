...
func ListBooking(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM Bookings").Preload("User").Find(&bookings).Error; err != nil {
...
func GetBooking(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM bookings WHERE id = ? ORDER BY room ASC", id).Preload("User").Find(&booking).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
